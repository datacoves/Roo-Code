	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version: string }) => {
			if (key === "chat:announcement.title") {
				return `🎉 Datacoves Copilot ${options?.version} Released`
			}
			if (key === "chat:announcement.description") {
				return `Datacoves Copilot ${options?.version} brings powerful new features and improvements based on your feedback.`
			}
...
describe("Announcement", () => {
	...
	it("renders the announcement with the version number from package.json", () => {
		...
		expect(screen.getByText(`🎉 Datacoves Copilot ${expectedVersion} Released`)).toBeInTheDocument()
		expect(
			screen.getByText(
				`Datacoves Copilot ${expectedVersion} brings powerful new features and improvements based on your feedback.`,
			),
		).toBeInTheDocument()
	})
})
