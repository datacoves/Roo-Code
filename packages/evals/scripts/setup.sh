...
build_extension() {
  echo "🔨 Building the Datacoves Copilot extension..."
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  ...
  echo "The Datacoves Copilot evals system can also be run with Docker on any platform."
...
if ! code --list-extensions 2>/dev/null | grep -q "Datacoves.datacoves-copilot"; then
  code --install-extension Datacoves.datacoves-copilot &>/dev/null || exit 1
fi
...
echo -n "🗄️ Syncing Datacoves Copilot evals database... "
...
read -p "💻 Do you want to build a new version of the Datacoves Copilot extension? [currently $current_version] (y/N): " build_extension
